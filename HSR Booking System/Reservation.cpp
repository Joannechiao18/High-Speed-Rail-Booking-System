...
{
    SouthboundTimetable southTrains;
    NorthboundTimetable northTrains;

    char stationNames[15][15]
        = { "Nangang", "Taipei", "Banqiao", "Taoyaun", "Hsinchu",
        "Miaoli", "Taichung", "Changhua", "Yunlin", "Chiayi", "Tainan", "Zuoying", };

    char carCodes[5][10]
        = { "Standard", "Business" };

    cout << endl << setw(10) << "Date" << setw(11) << "Train " << "No." << setw(9) << "From" << setw(11) << "To" << setw(12) << "Departure" << setw(10) << "Arrival" << setw(9) << "Adult" << setw(13) << "Concession" << setw(6) << "Fare" << setw(10) << "Class\n";
    cout << setw(10) << date << setw(14) << trainNumber << setw(9) << stationNames[originStation - 1] << setw(11) << stationNames[destinationStation - 1];

    if (destinationStation > originStation)
    {
        cout << setw(12) << southTrains.getDepartureTimes(trainNumber, originStation) << setw(10) << southTrains.getDepartureTimes(trainNumber, destinationStation);
    }
    else
    {
        /* for (int i = 0; i < northTrains.size(); i++)
             if (strcmp(trainNumber, northTrains[i].trainNumber) == 0)*/
        cout << setw(12) << northTrains.getDepartureTimes(trainNumber, destinationStation) << setw(10) << northTrains.getDepartureTimes(trainNumber, originStation);
    }

    int adultPrice, conPrice;
    if (destinationStation > originStation)
    {//«n¤U
        if (carClass == 1)
        {
            adultPrice = adultTicketPrice[destinationStation][originStation];
            conPrice = adultPrice / 2;
            cout << setw(7) << adultPrice << "*" << adultTickets << setw(11) << conPrice << "*" << concessionTickets << setw(6) << adultPrice * adultTickets + conPrice * concessionTickets << setw(9) << carCodes[0] << endl;
        }
        else if (carClass == 2)
        {
            adultPrice = adultTicketPrice[originStation][destinationStation];
            conPrice = adultPrice / 2;
            cout << setw(7) << adultPrice << "*" << adultTickets << setw(11) << conPrice << "*" << concessionTickets << setw(6) << adultPrice * adultTickets + conPrice * concessionTickets << setw(9) << carCodes[1] << endl;
        }
    }
    else
    {//¥_¤W
